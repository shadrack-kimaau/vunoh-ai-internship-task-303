...
def task_detail(request, task_code: str):
    task = get_object_or_404(Task, task_code=task_code)
    whatsapp = task.messages.filter(channel=TaskMessage.CHANNEL_WHATSAPP).first()
    email = task.messages.filter(channel=TaskMessage.CHANNEL_EMAIL).first()
    sms = task.messages.filter(channel=TaskMessage.CHANNEL_SMS).first()
    ...
    return JsonResponse(
        {
            "task": {
                ...
                "customer_request": task.customer_request,
                "intent": task.intent,
                "entities": task.entities,
                "risk_score": task.risk_score,
                "status": task.status,
                "assigned_team": task.assigned_team,
                "created_at": task.created_at.isoformat(),
                "reference_task_code": task.reference_task_code,
            },
            "steps": [
                {"step_order": s.step_order, "step_text": s.step_text} for s in task.steps.all()
            ],
            "messages": {
                "whatsapp": whatsapp.message_text if whatsapp else "",
                "email": {
                    "subject": email.subject if email else "",
                    "body": email.message_text if email else "",
                },
                "sms": sms.message_text if sms else "",
            },
            "status_history": [
                {
                ...
                for h in task.status_history.all()
            ],
        }
    )
    ...
    if intent == Task.INTENT_CHECK_STATUS:
        ...
        if reference_task_code:
            ...
            if ref:
                ...
                # Provide this snapshot to AI generators (and to fallback templates).
                entities = dict(entities)
                entities["reference_task"] = reference_task_snapshot
